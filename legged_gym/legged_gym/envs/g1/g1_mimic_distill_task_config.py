...
from legged_gym.envs.g1.task_obs_defs import NUM_TASK_BODIES, TASK_MIMIC_OBS_DIM
NUM_KEY_BODIES = 9
...
class G1MimicDistillTaskPrivCfg(HumanoidMimicCfg):
    class env(HumanoidMimicCfg.env):
        ...
        n_priv_info = (
            3 + 1 + 3 * NUM_KEY_BODIES + 2 + 4 + 1 + 2 * num_actions
        )  # base lin vel, root height, key body pos, contact mask, priv latent
        history_len = 10
    ...
    class domain_rand:
        domain_rand_general = (
            True  # manually set this, setting from parser does not work;
        )
...
class G1MimicDistillTaskStuCfg(G1MimicDistillTaskPrivCfg):
    class env(G1MimicDistillTaskPrivCfg.env):
        tar_obs_steps = [
            1,
            5,
            10,
...
class G1MimicDistillTaskStuRLCfg(G1MimicDistillTaskPrivCfg):
    class env(G1MimicDistillTaskPrivCfg.env):
        tar_obs_steps = [
            1,
            5,
...
class G1MimicDistillTaskPrivCfgPPO(HumanoidMimicCfgPPO):
    seed = 1
    ...
    class runner(HumanoidMimicCfgPPO.runner):
        ...
        # logging
        save_interval = (
            500  # check for potential saves every this many iterations
        )
...
class G1MimicDistillTaskStuRLCfgDAgger(G1MimicDistillTaskStuRLCfg):
    seed = 1

    class teachercfg(G1MimicDistillTaskPrivCfgPPO):
        pass

    class runner(G1MimicDistillTaskPrivCfgPPO.runner):
        policy_class_name = "ActorCriticMimic"
        ...
        teacher_proj_name = "g1_priv_mimic_task"
    ...
    class algorithm(HumanoidMimicCfgPPO.algorithm):
        ...
        dagger_coef_anneal_steps = (
            60000  # Total steps to anneal dagger_coef to dagger_coef_min
        )
